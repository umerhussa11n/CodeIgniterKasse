...
class SSP {
     ...
     *
     */
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '', $page = '') {
        $bindings = array();
        ...
        if ($joinQuery) {
            ...
            $query2 = "SELECT COUNT({$table}.{$primaryKey})
			 $joinQuery
			 $where
			 $extraWhere
			 $groupBy
       ...
       $having
            ...
            $query2 = "SELECT COUNT({$primaryKey})
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        //print_r($data);die;
        // Data set length after filtering //SQL_CALC_FOUND_ROWS
        $resFilterLength = SSP::sql_exec($db, $bindings, $query2);
        $recordsFiltered = $resFilterLength[0][0];
        // Total data set length
        $resTotalLength = SSP::sql_exec($db, "SELECT COUNT(`{$primaryKey}`)
			 FROM   `$table` WHERE $extraWhere_org");
        ...
        if ($page == "access") {
          $CI->load->model('device_model');
            for ($i = 0, $total = count($all_data["data"]); $i < $total; $i++) {
                $all_data["data"][$i]['item_name'] = $CI->device_model->get_item_names_concat($all_data["data"][$i][0]);
                $all_data["data"][$i][2] = date("d/m/Y H:i", $all_data["data"][$i][2]);
                $all_data["data"][$i][3] = $all_data["data"][$i][3]."<br />".$all_data["data"][$i][7];
                $all_data["data"][$i][4] = $CI->global_model->payment_type($all_data["data"][$i][4]);
                $all_data["data"][$i]['profit'] = number_format($CI->global_model->calculate_earnings_on_phone($all_data["data"][$i][0]), 2, ',', '.');
				$all_data["data"][$i]['new_comment'] = $CI->device_model->check_new_comment($all_data["data"][$i][0]);
				$all_data["data"][$i]['comment_count'] = $CI->db->from('order_comment')->where('order_id',$all_data["data"][$i][0])->count_all_results();
            }
        ...
        if ($page == "bestilling") {
          for ($i = 0, $total = count($all_data["data"]); $i < $total; $i++) {
            $all_data["data"][$i][2] = date("d/m/Y H:i", strtotime($all_data["data"][$i][2]));
          }
        }
        ...
        if ($page == "receipt") {
          for ($i = 0, $total = count($all_data["data"]); $i < $total; $i++) {
			...
			$all_data["data"][$i]['product_name'] = $CI->global_model->get_product_name($all_data["data"][$i][3]);
          }
        }
        ...
        if ($page == "transfer") {
          for ($i = 0, $total = count($all_data["data"]); $i < $total; $i++) {
              ...
              $all_data["data"][$i][3] = date("d/m/Y H:i", $all_data["data"][$i][3]);
